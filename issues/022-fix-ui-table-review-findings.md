...
6. 7 estilos inline via `style` em `ingredientsTable.ts` (~linha 106) — extrair para classes utilitárias com tokens, documentadas. **Ampliado pela review da 015**: +8 ocorrências em `sourdoughTable.ts` (linhas ~124/134/207/285/301/302/576/591) — total 15; extrair todas para classes utilitárias tokenizadas (ex. `.mb-3`, `.row-gap-2`) documentadas em `references/design-system.html`.
