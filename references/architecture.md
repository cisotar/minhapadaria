...
| `src/core/recalc.ts` | **Engine central de recálculo em lote** — UMA função `recalculate` que reconstrói TODOS os valores derivados de uma Recipe a partir do estado puro (§1.6). Orquestra sem duplicar: bakers (§3.A) · sourdough (§3.B) · hydration (§2.C/§2.D) · costs (§3.E) · pricing (§3.E/§3.E decisão 4). Suporta 2 modos: (a) %→peso — derivar pesos a partir das %, reusando bakers.weightFromPercentage; (b) peso→% — calcular % como peso/total geral (INCLUI W_ferm §1.3), reusando bakers.percentageFromWeight; implementa transitionToPercentageMode (§1.5) para conversão explícita. Sem DOM, sem localStorage, sem arredondamento (§9), pureza total. Tipos ampliados: HydrationSummary/RecipeSummary com `number | null` (contrato null≠0); Pricing.priceInputMode novo (`'sale-price'|'margin'|'profit'`). | §1.2, §1.3, §1.5, §1.6, §3.A, §3.B, §3.E, §2.C, §2.D, §9 |
| `src/core/recalc.test.ts` | TDD: 7 casos engine central — modo %→peso (deriva pesos de %), modo peso→% (denominador fermento+principal), transição peso→%→peso (F_total dos pesos atuais), idempotência ambos modos, pureza (entrada não mutada), null defensivo (custo impossível não colapsa a 0). Golden §12 validado ponta a ponta (unitCost 4,43; preço 7,3833; margem 40; hidratação real 72,7273%; farinha real 1100g). | §1.2, §1.3, §1.5, §1.6, §3.A, §3.B, §3.E, §2.C, §2.D, §5.C, §9, §12 |
